...
class Inventario(tk.Frame, Modulos):
    ...
    #--------------------------------------------------------------------------------------------------------
        canvas_articulos = tk.LabelFrame(self, text="Articulos", font="arial 14 bold", bg="#95c799")
        canvas_articulos.place(x=300, y=10, width=780, height=580)
        
        self.canvas = tk.Canvas(canvas_articulos, bg="#95c799")
    ...
    #-------------------------------------------------------------------------------------------------------
        ...
        lblframe_seleccion = LabelFrame(self, text="Selección", font="arial 14 bold", bg="#95c799")
        lblframe_seleccion.place(x=10, y=95, width=280, height=270)
        
        self.label7 = tk.Label(lblframe_seleccion, text="Categorias", font="arial 12", bg="#95c799")
        self.label7.place(x=5,y=5)
        
        self.label1 = tk.Label(lblframe_seleccion, text="Articulo:", font="arial 12", bg="#95c799", wraplength=150)
        self.label1.place(x=5,y=40)
        
        self.label2 = tk.Label(lblframe_seleccion, text="Precio:", font="arial 12", bg="#95c799")
        self.label2.place(x=5,y=70)
        
        self.label3 = tk.Label(lblframe_seleccion, text="Costo:", font="arial 12", bg="#95c799")
        self.label3.place(x=5,y=100)
        
        self.label4 = tk.Label(lblframe_seleccion, text="Stock:", font="arial 12", bg="#95c799")
        self.label4.place(x=5,y=130)

        self.label5 = tk.Label(lblframe_seleccion, text="Estado:", font="arial 12", bg="#95c799")
        self.label5.place(x=5,y=160)
        
    #-------------------------------------------------------------------------------------------------------

        lblframe_botones = LabelFrame(self, bg="#95c799", text="Opciones", font="arial 14 bold")
    ...
    def sumar_stock(self):
        ...
        tk.Label(top, text=f"Artículo: {articulo_seleccionado}", font="arial 12 bold", bg="#95c799").pack(pady=5)
        
        stock_label = tk.Label(top, text=f"Stock: {stock_actual}", font="arial 12 bold", bg="#95c799")
        stock_label.pack(pady=5)

        tk.Label(top, text="Escanea el nuevo código de barras:", font="arial 12 bold", bg="#95c799").pack(pady=5)
    ...
    def agregar_articulo(self):
        ...
        top.geometry("700x400+200+50")  # Reduje un poco la altura ya que no estará el código de barras
        top.config(bg="#95c799")
        top.resizable(False, False)
        ...
        tk.Label(top, text="Categoría:", font="arial 12 bold", bg="#95c799").place(x=20, y=20, width=150, height=25)
        ...
        tk.Label(top, text="Artículo:", font="arial 12 bold", bg="#95c799").place(x=20, y=70, width=150, height=25)
        entry_articulo = ttk.Entry(top, font="arial 12 bold")
        entry_articulo.place(x=180, y=70, width=250, height=30)

        # Campo para el precio
        tk.Label(top, text="Precio:", font="arial 12 bold", bg="#95c799").place(x=20, y=120, width=150, height=25)
        entry_precio = ttk.Entry(top, font="arial 12 bold")
        entry_precio.place(x=180, y=120, width=250, height=30)

        # Campo para el costo
        tk.Label(top, text="Costo:", font="arial 12 bold", bg="#95c799").place(x=20, y=170, width=150, height=25)
        ...
        tk.Label(top, text="Stock:", font="arial 12 bold", bg="#95c799").place(x=20, y=220, width=150, height=25)
        stock_label = tk.Label(top, text="Stock: 0", font="arial 12 bold", bg="#95c799")  # Stock empieza en 0
        stock_label.place(x=180, y=220, width=250, height=30)

        # Campo para el estado
        tk.Label(top, text="Estado:", font="arial 12 bold", bg="#95c799").place(x=20, y=270, width=150, height=25) # Ajusté la posición en Y
        ...
        self.default_image = Image.open("fotos/default.png")  # Asegúrate de que la ruta sea correcta
        ...
        self.frameimg_label = default_image_label # Guarda una referencia al label de la imagen
        ...
        def guardar():
            ...
            try:
                ...
                self.cur.execute(
                    "INSERT INTO inventario (producto_id) VALUES (?)", # Ya no insertamos código de barras aquí
                    (producto_id,)
                )
                self.con.commit()
                ...
                combobox_estado.set("")
                self.image_path = "fotos/default.png" # Restablecer la ruta de la imagen
                self.frameimg_label.config(image=self.default_image_tk) # Mostrar la imagen por defecto
                self.frameimg_label.image = self.default_image_tk # Mantener la referencia
        ...
        entry_articulo.focus() # Establecer el foco inicial en el campo del artículo
    ...
    def cargar_articulos(self, filtro=None, categoria=None):
        try:
            ...
            # Aplicar filtros según se especifique
            if filtro:
            ...
            # Actualizar lista interna con los nombres de los artículos
            self.articulos = [articulo[1] for articulo in articulos] if articulos else []

            # Limpiar los widgets existentes en el scrollable_frame
            for widget in self.scrollable_frame.winfo_children():
                widget.destroy()

            # Mostrar mensaje si no hay artículos que cargar
            if not articulos:
                tk.Label(self.scrollable_frame, text="No hay artículos que mostrar", bg="white", font="arial 12 italic").pack()
                return

            # Inicializar posición de fila y columna
    ...
    def mostrar_articulo(self, categoria, articulo, precio, stock, image_path=None):
        articuloframe = tk.Frame(self.scrollable_frame, bg="white", relief="solid")
        ...
        if image_path and os.path.exists(image_path): 
            ...
            image_label = tk.Label(articuloframe, image=imagen, bg="white")
            image_label.image = imagen
            image_label.pack(expand=True, fill="both")
            image_label.bind("<Button-1>", lambda event: self.mostrar_click(articulo))
            image_label.pack(expand=True, fill="both")

        category_label = tk.Label(articuloframe, text=f"Categoría: {categoria}", bg="white", anchor="w", wraplength=150, font="arial 10 bold")
        category_label.pack(side="top", fill="both")

        name_label = tk.Label(articuloframe, text=f"Nombre: {articulo}", bg="white", anchor="w", wraplength=150, font="arial 10 bold")
        name_label.pack(side="top", fill="both")

        price_label = tk.Label(articuloframe, text=f"Precio: ${precio:.2f}", bg="white", anchor="w", wraplength=150, font="arial 10")
        price_label.pack(fill="x", anchor="w")

        stock_label = tk.Label(articuloframe, text=f"Stock: {stock}", bg="white", anchor="w", wraplength=150, font="arial 10")
    ...
    def mostrar_click(self, articulo):
        try:
            ...
            # Ejecuta la consulta para obtener los datos del producto, incluyendo el código de barras
    ...
    def _filter_articulos(self):
        try:
            ...
            # Si self.articulos está vacío, recargar los datos
            if not self.articulos:
                self.articulos_combobox()  # Actualiza los artículos desde la base de datos

            if typed == '':
                data = self.articulos  # Muestra todos los artículos
            else:
    ...
    def editar_articulos(self):
        ...
        # Campo para el estado
        tk.Label(top, text="Estado:", font="arial 12 bold", bg="#95c799").place(x=20, y=220, width=150, height=25)
        combobox_estado = ttk.Combobox(top, font="arial 12 bold", values=["Activo", "Inactivo"], state="readonly")
        ...
        def guardar():
            ...
            try:
                ...
                # Después de actualizar un producto, recalcula el stock de la categoría
                self.cur.execute(
    ...
    def eliminar_articulo(self):
        ...
        if confirmar:
            try:
                ...
                categoria = categoria_resultado[0]  # Asigna la categoría correspondiente
                ...
                # Si ya no quedan artículos en la categoría, establece el stock como 0
                self.cur.execute(
                ...
                # Actualizar la lista y el combobox después de eliminar
